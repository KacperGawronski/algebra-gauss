...
#define DATA_TYPE npy_cdouble
#define DATA_TYPE_DEFINE NPY_CDOUBLE
#define DATA_TYPE_EPSILON FLT_EPSILON
#define SUPPORT_DATA_TYPE npy_double

void npy_cdouble_multiply_matrixes(DATA_TYPE *A,int **indexes,DATA_TYPE *data, DATA_TYPE *result,npy_intp *dims,int start){
 	int i,j,l,index;
 	if(start>0){
...
PyObject* npy_cdouble_eliminate(PyObject *raw_arr,PyObject *raw_vals){
	...
	for(i=0;i<dims[0];++i){
		...
		if(current_row<dims[0]){
			...
			npy_cdouble_multiply_matrixes(A,indexes,data1,data2,dims,i);
			swapper=data1;
			data1=data2;
			data2=swapper;
		}else{
